...
{
    public partial class CleanAndWirePublisherFk : Migration
    {
...
SET @create_idx_sql := IF(@idx_exists = 1, 'SELECT 1',
  'CREATE INDEX `IX_Books_PublisherId` ON `Books` (`PublisherId`)');
PREPARE stmt FROM @create_idx_sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
");
...
SET @drop_fk_sql := IF(@fk_name IS NULL, 'SELECT 1',
  CONCAT('ALTER TABLE `Books` DROP FOREIGN KEY `', @fk_name, '`'));
...
SET @drop_idx_sql := IF(@idx_exists = 1,
  'DROP INDEX `IX_Books_PublisherId` ON `Books`', 'SELECT 1');
